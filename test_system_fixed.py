...
class SystemTester:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "client-id" in response.text and "未分配" not in response.text:
                    ...
                    self.log_test("客户端控制台访问", False, "客户端ID显示为'未分配'")                else:
    ...
    def test_api_endpoints(self):
        ...
        try:
            # 登录服务器端
            login_response = server_session.post(f"{BASE_URL}/login",
                                               json=server_login,
                                               headers={'Content-Type': 'application/json'})
            ...
            if login_response.status_code == 200 and login_response.json().get('success'):
                # 测试服务器节点API
                try:
                    response = server_session.get(f"{BASE_URL}/api/server_nodes")
                    if response.status_code == 200:
                        data = response.json()
                ...
                # 测试服务器日志API
                try:
                    response = server_session.get(f"{BASE_URL}/api/server_logs")
                    if response.status_code == 200:
                        data = response.json()
                        if data.get('success') and isinstance(data.get('logs'), list):
                            self.log_test("服务器日志API", True, f"返回 {len(data['logs'])} 条日志")
                        else:
                ...
                self.log_test("服务器节点API", False, "无法登录服务器端测试")
                self.log_test("服务器日志API", False, "无法登录服务器端测试")
                
        except Exception as e:
            self.log_test("服务器节点API", False, f"服务器端登录失败: {str(e)}")
            self.log_test("服务器日志API", False, f"服务器端登录失败: {str(e)}")
    ...
    def test_error_handling(self):
        ...
        # 测试重复用户名注册
        duplicate_user = {
            "username": self.client_username,  # 已存在用户名
            "password": "test123456",
            "confirmPassword": "test123456",
            "account_type": "client"
        }
